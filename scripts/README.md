...
| `release.sh` | Creates a GitHub release with whatever dist artifacts exist (DMG, ZIP, EXE, AppImage, etc.) for the current `package.json` version. Uses `release-notes.md` if present (generated by `/release-notes` skill), otherwise falls back to GitHub auto-generated notes. Run via `pnpm release`. |
| `release-all.sh` | Full release pipeline: runs checks, bumps version, builds, signs, and publishes. Uses `release-notes.md` for release body if present. Run via `pnpm release:all <patch\|minor\|major>`. |
