...
- **IMPORTANT — Links**: Every mention of a file, function, or code location MUST be a markdown link to the PR diff view. The link text should be the file path (with optional line range), and the URL should point to the diff. Never paste raw URLs — always use \`[file:lines](url)\` format. Never omit links — the reader navigates the review by clicking them. All links MUST point to the PR diff view, never to the blob/file view. ${prChangesUrl ? `Use \`${prChangesUrl}\` as the base URL and append \`#diff-<sha256-of-filepath>\` for each file.` : 'Use the PR files/changes URL (e.g. `https://github.com/owner/repo/pull/N/files#diff-<sha256-of-filepath>`) for each file.'} To compute the anchor, take the SHA-256 hash of the file's relative path (run \`echo -n "path" | shasum -a 256\`). Example: \`[src/app.tsx:12-45](${prChangesUrl || 'https://github.com/owner/repo/pull/N/files'}#diff-<sha256 of "src/app.tsx">)\`.
